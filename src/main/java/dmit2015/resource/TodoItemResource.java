...
// Default media types. @Consumes and @Produces can be placed in front of Classes or Methods.
// If they are in front of Classes, all methods in that Class inherit the notations.
@Consumes(MediaType.APPLICATION_JSON)    // All methods this class accept only JSON format data
@Produces(MediaType.APPLICATION_JSON)    // All methods returns data that has been converted to JSON format
public class TodoItemResource {

    // Jaxrs will populate this for us.
    @Context
    private UriInfo uriInfo;

    // Dependency injection. Container CDI will create this object for us when this object is being called.
    // @Inject means this object is created the first we access this variable.
    @Inject
    ...
    @Path("{id}")                //  These two ids in @Path("{id}") and @PathParam("id") have to match.
